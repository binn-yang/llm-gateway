...
impl AnthropicOAuthQuotaProvider {
    ...
    /// Shared implementation for both ProviderInstanceConfig and AnthropicInstanceConfig.
    async fn query_quota_impl(
        &self,
        instance_name: &str,
        auth_mode: &AuthMode,
        provider_name: &str,
        ...
        // 2. 获取 access token
        let token = match self.token_store.get_token("anthropic").await {
            Ok(t) => t,
            Err(e) => {
...
impl QuotaProvider for AnthropicOAuthQuotaProvider {
    async fn query_quota(
        ...
        self.query_quota_impl(&instance.name, &instance.auth_mode, provider_name).await
    }
